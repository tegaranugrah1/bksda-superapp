...
import { LeaveRequestPrintData } from "@/components/FormulirCutiPrintModal";

interface EditCutiModalProps {
  visible: boolean;
...
export const EditCutiModal: React.FC<EditCutiModalProps> = ({
  visible,
  data,
  onClose,
  onSuccess,
  ...
  useEffect(() => {
    if (data) {
      setJenisCuti(data.jenis_cuti || "Cuti Tahunan");
      setAlasanCuti(data.alasan_cuti || "");
      setTanggalMulai(data.tanggal_mulai || "");
      setTanggalSelesai(data.tanggal_selesai || "");
      setAlamat(data.alamat_menjalankan_cuti || "");
  ...
  const handleSubmit = async () => {
    if (!alasanCuti.trim()) {
      Alert.alert("Form Tidak Lengkap", "Mohon isi alasan permohonan cuti.");
      return;
    }
    if (!alamat.trim()) {
    ...
    try {
      await apiClient.put(`/kepegawaian/leave-requests/${data.id}`, {
        ...
        masa_kerja: masaKerja.trim() || undefined,
      });

      Alert.alert(
        "Berhasil Diperbarui",
        "Data pengajuan cuti pegawai telah berhasil diperbarui."
      );
      onSuccess();
      onClose();
    } catch (err: any) {
      const msg = err.response?.data?.message || err.response?.data?.error || "Gagal mengedit pengajuan cuti.";
      Alert.alert("Gagal Edit", msg);
    } finally {
      ...
      <View style={styles.overlay}>
        <View style={styles.contentContainer}>
          ...
          <View style={styles.header}>
            <View>
              <Text style={styles.headerTitle}>Edit Pengajuan Cuti</Text>
              <Text style={styles.headerSubtitle}>
                Pegawai: {data.employee?.nama_lengkap || "-"}
              </Text>
            </View>
          ...
          <ScrollView style={styles.formScroll} showsVerticalScrollIndicator={false}>
            {/* 1. Jenis Cuti */}
            <Text style={styles.label}>1. Jenis Cuti</Text>
            ...
            <View style={styles.dateRow}>
              ...
              <View style={{ flex: 1, marginLeft: 6 }}>
                <Text style={styles.label}>Masa Kerja</Text>
                <TextInput
                  style={styles.input}
                  placeholder="Contoh: 2 Tahun 0 Bulan"
                  placeholderTextColor="#94a3b8"
                  value={masaKerja}
                  onChangeText={setMasaKerja}
                />
              </View>
            </View>

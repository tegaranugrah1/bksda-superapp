...
class PowerOfAttorneyController extends Controller
{
    ...
    public function update(Request $request, string $id): JsonResponse
    {
        ...
        $ktpPath = $agreement->ktp_path;
        if ($request->hasFile('ktp_image')) {
            if ($ktpPath && \Illuminate\Support\Facades\Storage::exists($ktpPath)) {
                \Illuminate\Support\Facades\Storage::delete($ktpPath);
            }
            $file = $request->file('ktp_image');
            $firstPartyName = $validated['first_party']['name'];
            $sanitizedName = \Illuminate\Support\Str::slug($firstPartyName, '_');
        ...
        return response()->json([
            'message' => 'Surat Kuasa Kendaraan berhasil diperbarui.',
            'data' => new PowerOfAttorneyResource($agreement->fresh(['employee', 'generator'])),
        ]);
    }
}

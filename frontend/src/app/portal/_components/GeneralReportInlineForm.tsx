...
function formatJudulLaporan(rawText: string): string {
  ...
  // If contains "dalam rangka", extract the activity name after "dalam rangka"
  ...
  if (matchDalamRangka && matchDalamRangka[1]) {
    ...
    // Strip leading words: "melaksanakan kegiatan", "melaksanakan perjalanan dinas", "menugaskan staf", "melaksanakan"
...
export function GeneralReportInlineForm({ onBack }: GeneralReportInlineFormProps) {
  ...
  const handleSelectSuratTugas = async (stId: string) => {
    ...
    try {
      ...
      // 1. Judul Laporan Auto (Strips "Melaksanakan kegiatan / Perjalanan dinas")
      setJudulLaporan(formatJudulLaporan(rawMaksud));

      // 2. Agenda Pelaksanaan Auto (Full intent sentence)

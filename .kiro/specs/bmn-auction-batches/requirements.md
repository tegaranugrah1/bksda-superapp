...
- **Status Lifecycle**: Batch lelang memiliki 5 status utama yang dikendalikan oleh mesin status backend:
  ...
  - `DIAJUKAN`: Berkas dikunci dan diajukan ke instansi terkait (KL/KPKNL). Pengguna mencatat `no_surat_persetujuan` dan `tanggal_surat_persetujuan`. Aset diubah status penggunaannya menjadi dihentikan (`henti_guna = true`) untuk mencegah transaksi lain.
  ...
  - `BATAL`: Batch dibatalkan. Aset yang ada dalam batch dilepas kembali ke pool kandidat rusak berat, dan status penggunaan dikembalikan ke semula.
...
Alasan:
- **Kepatuhan Regulasi DJKN/KPKNL**: Aset dipaketkan dalam Lot, memiliki batas harga limit (Nilai Taksiran), melacak Surat Persetujuan BMN dan Surat Penetapan KPKNL secara eksplisit di database.
...
- **Siklus Hidup Aset Otomatis**: Integrasi langsung dengan modul disposal untuk menuliskan status soft-delete, mencatat tanggal penghapusan resmi (`tanggal_pengapusan`), menghentikan penggunaan sementara saat pengajuan (`henti_guna`), dan mencatat riwayat perubahan (`AssetUpdate`).
...
- **Penghapusan Otomatis (Auto-Disposal)**: Proses pengubahan status aset menjadi non-aktif (`Dihapus/Pemutihan`), pencatatan `tanggal_pengapusan`, dan soft-delete dari database secara otomatis setelah dinyatakan terjual.
...
### Requirement 4: Freezing Signatories and Document Metadata & Asset Freeze
**User Story:** Sebagai BMN Operator, saya ingin memilih pejabat penandatangan dan membekukan data profil mereka ke dalam batch lelang, serta membekukan status operasional aset terkait, agar berkas legal dan inventaris sinkron.

#### Acceptance Criteria
1. WHEN batch berstatus `DRAFT`, THE BMN Operator SHALL dapat memilih pegawai yang bertindak sebagai Kepala Balai, Panitia, Tim Penilai, dan Pemeriksa.
2. WHEN status batch berubah dari `DRAFT` menjadi `DIAJUKAN`, THE Backend SHALL mengambil snapshot data profil para penandatangan tersebut dan menyimpannya ke dalam kolom JSON `metadata` pada tabel `bmn_auction_batches`.
3. WHEN status batch dikunci menjadi `DIAJUKAN`, THE Backend SHALL secara otomatis mengupdate kolom `status_penggunaan` pada tabel `bmn_assets` menjadi `'Dihentikan dari Penggunaan Dinas'` dan kolom `henti_guna` menjadi `true` untuk seluruh aset yang terikat di dalam batch tersebut.
4. AFTER status batch berubah menjadi `DIAJUKAN` atau status di atasnya, THE System SHALL menggunakan data dari kolom `metadata` tersebut untuk merender dokumen cetak, bukan mengambil secara real-time dari tabel pegawai aktif.
5. THE System SHALL mencatat seluruh nomor surat yang diinput oleh operator ke dalam kolom JSON `metadata` saat batch dikunci ke status `DIAJUKAN`.

### Requirement 5: KPKNL Auction Approval & Scheduling
**User Story:** Sebagai BMN Admin, saya ingin mencatat nomor persetujuan lelang KPKNL dan menetapkan jadwal pelaksanaan lelang dari KPKNL, agar pelacakan proses lelang berjalan sesuai hukum.

#### Acceptance Criteria
1. WHEN batch lelang berstatus `DIAJUKAN`, THE BMN Admin SHALL dapat menaikkan status menjadi `JADWAL_DITETAPKAN`.
2. WHEN menaikkan status menjadi `JADWAL_DITETAPKAN`, THE BMN Admin SHALL diwajibkan menginput field `no_surat_persetujuan` (nomor surat persetujuan penjualan BMN dari KPKNL/KSDAE), `tanggal_surat_persetujuan`, `no_surat_penetapan` (nomor surat jadwal lelang KPKNL), dan `tanggal_lelang`.
3. IF salah satu dari keempat field tersebut kosong, THEN THE Backend SHALL menolak transisi status dan mengembalikan error validasi.

### Requirement 6: Realization Recording, Deletion Date & Auto-Disposal
**User Story:** Sebagai BMN Admin, saya ingin mencatat hasil lelang per aset (apakah terjual dan harga terbentuknya) serta memicu penghapusan otomatis aset yang terjual beserta pengisian tanggal penghapusan resmi, agar buku neraca aset Balai terupdate secara otomatis dan akurat.

#### Acceptance Criteria
1. WHEN batch berstatus `JADWAL_DITETAPKAN`, THE BMN Admin SHALL dapat mengubah status batch menjadi `REALISASI`.
2. WHEN memproses transisi ke `REALISASI`, THE BMN Admin SHALL diwajibkan mengisi hasil lelang untuk tiap-tiap aset di dalam batch: `is_sold` (Boolean) dan `harga_terbentuk` (Decimal, wajib diisi jika `is_sold` bernilai true).
3. WHEN status batch resmi disimpan sebagai `REALISASI`:
   - THE Backend SHALL memperbarui kolom `tanggal_pengapusan` pada tabel `bmn_assets` dengan nilai `tanggal_lelang` milik batch lelang terkait untuk setiap aset dengan `is_sold = true`.
   - THE Backend SHALL memicu pemanggilan fungsi disposal (`AssetService::disposeAsset()`) untuk setiap aset dengan `is_sold = true`.
   ...
   - THE Backend SHALL membiarkan aset dengan `is_sold = false` tetap berstatus aktif (`kondisi = Rusak Berat`, status di tabel `bmn_assets` tetap `Aktif`, soft-delete tidak dipicu) serta mengembalikan `henti_guna = false` dan `status_penggunaan` kembali ke semula agar dapat dimasukkan kembali ke batch lelang berikutnya.
...
**User Story:** Sebagai BMN Admin, saya ingin membatalkan batch lelang yang sedang berjalan, agar semua aset di dalamnya dibebaskan kembali menjadi kandidat rusak berat serta dilepaskan pembekuannya.
...
2. WHEN status berubah menjadi `BATAL`, THE Backend SHALL melepas relasi aktif aset dan mengembalikan kolom `henti_guna` menjadi `false` serta `status_penggunaan` kembali ke kondisi semula untuk semua aset di dalam batch.

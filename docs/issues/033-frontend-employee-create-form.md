...
> **Type**: `feature`
> **Labels**: `frontend`, `ui`, `form`, `upload`
> **Priority**: 🔴 Critical (Gerbang utama penambahan sumber daya manusia)
> **Complexity**: 🔴 High (Penanganan State `FormData`, Mutasi Cache, dan Pratinjau Foto)
> **Recommended AI Model**: Claude Sonnet / Gemini 2.5 Pro / GPT-4o
> **Dependencies**: Issue #032 (Employee List View)
...
```
issue/033-frontend-employee-create-form
```

## Deskripsi

Data tabel dari Issue 032 masih kosong. Tentu saja, kita butuh "Pabrik" atau formulir untuk memproduksinya! Dalam membangun form modern standar kelas *Enterprise*, kita tidak boleh lagi mengandalkan tag standar HTML yang me-*refresh* keseluruhan halaman web (kuno).

Sesuai dengan **Rule 7.4 (Validasi Dua Sisi)** dan tuntutan desain estetika tinggi BKSDA, Form ini akan dipersenjatai dengan kemampuan manipulasi gambar *(Image Preview)*. Ketika pengguna memilih sebuah file Pas Foto dari komputer, kita tidak langsung melemparnya ke server. Kita buatkan URL lokal gaib (*Blob Object URL*) agar fotonya langsung tersaji *(preview)* di dalam bingkai layar dengan seketika.

Setelah selesai, form tidak mengirim data JSON biasa, melainkan membungkusnya dalam paket `FormData` secara diam-diam (Asynchronous) melalui mesin gaib `useMutation` milik TanStack. Begitu server menjawab "Sukses", TanStack secara otomatis akan mem-bypass sistem dan *menghapus Cache* Tabel (Issue 032) agar data ter-update seketika layaknya sihir.

---

## Acceptance Criteria

- [ ] Folder rute `kepegawaian/create` dan `page.tsx` dibuat.
- [ ] Tersedia pratinjau gambar *(Image Preview)* interaktif saat memilih Foto.
- [ ] Dilengkapi proteksi lapis depan **(Rule 4.1 & 4.2)**: Blokir file bukan gambar dan blokir ukuran di atas 10MB langsung di sisi *browser* (tanpa membebani server).
- [ ] Proses *Submit* dibungkus ke dalam objek `FormData` (Karena mengirimkan file biner).
- [ ] Menggunakan `queryClient.invalidateQueries` untuk me-*refresh* tabel secara gaib *(Seamless UX)* lalu dikembalikan ke halaman *List Pegawai*.

---

## Langkah Demi Langkah

> 💡 **Untuk junior/AI**: Pendekatan UI kita menggunakan gaya "Bagi Dua" *(Split Layout)* layaknya aplikasi pendaftaran *(Onboarding)* Premium. Kiri khusus bingkai Foto, kanan khusus Form Teks. Jangan pernah menggunakan tag HTML `form action="..."` murni, gunakanlah `onSubmit={handleSubmit}` yang dirangkai dengan fungsi bawaan React `e.preventDefault()`.

### Langkah 1: Bangun Kanvas Form Pengisian Data

**Kenapa?** Form ini harus bisa menangani state lokal (React `useState`) untuk memantau teks NIP dan sekaligus melacak file *binary* foto.

```bash
mkdir -p frontend/src/app/\(dashboard\)/kepegawaian/create
```

**Path:** `e:\bksda-superapp\frontend\src\app\(dashboard)\kepegawaian\create\page.tsx`

**Buat file baru tersebut, dan salin konfigurasi ajaib di bawah ini dengan presisi tingkat dewa:**
...
export default function EmployeeCreatePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-zinc-900 border border-zinc-200/80 dark:border-zinc-800 rounded-[2rem] shadow-xl shadow-zinc-200/40 dark:shadow-black/40 overflow-hidden flex flex-col md:flex-row">
             ...
             <div className="space-y-2.5 md:col-span-2">
               ...
               <div className="relative">
                 ...
                 <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none">
                    <ChevronDown className="w-4 h-4 text-zinc-500" /> {/* Pastikan mengimport ChevronDown dari lucide-react */}
                 </div>
               </div>
             </div>
...
### Q: Tombol sudah ditekan tapi muncul pesan error "Payload is not a valid JSON".

**Artinya:** Modul API Interceptor kamu secara otomatis memaksakan `Content-Type: application/json` pada header.
**Solusi:** Kodingan di atas sangat sakti karena merancang *Request* dengan header `multipart/form-data`. Axios di `lib/api.ts` kamu harusnya secara otomatis menerima modifikasi ini. Jika masih error, pastikan versi Axios sudah *Up to Date* karena Axios otomatis menyesuaikan mode payload berdasarkan objek `FormData` yang diterima.

### Q: Kenapa *Preview Image* tidak bisa tampil di Safari/Edge versi lama?

**Artinya:** Komputermu tidak memiliki memori DOM yang mumpuni untuk membuat Blob Virtual.
**Solusi:** Trik penggunaan `URL.createObjectURL(file)` adalah sihir Javascript tingkat tinggi. Browser era purba mungkin tidak mendeteksinya. Abaikan hal ini, karena BKSDA SuperApp akan dijalankan pada peramban Chrome modern.
...
gh issue create \
  --title "feat(kepegawaian): data entry form and photo upload" \
  --body "Implementasi Formik/RHF tingkat lanjut menggunakan FormData payload dan caching invalidation (React Query). Detail di docs/issues/033-frontend-employee-create-form.md" \
  --label "frontend,ui,form,upload"
```
...
git checkout -b issue/033-frontend-employee-create-form
```

### Step 3: Kerjakan

Salin kodingan ini ke folder baru yang letaknya berada "di dalam" rute Kepegawaian (karena ini adalah Sub-Rute / `nested route`). Tambahkan impor `import { ChevronDown } from "lucide-react";` di bagian baris atas *(Tadi hampir tertinggal)*.
...
cd e:\bksda-superapp
git add frontend/
git commit -m "feat(kepegawaian): data entry form and photo upload (#33)"
git push -u origin issue/033-frontend-employee-create-form
```
...
```bash
gh pr create \
  --title "feat(kepegawaian): data entry form and photo upload (#33)" \
  --body "## Summary
Membuka kanal masukan *(Input Channel)* untuk menambah populasi data Pegawai ke dalam SuperApp.

## Changes
- Pembuatan Rute Form \`/kepegawaian/create\`.
- Mekanisme Interaktif Kotak Pengambilan Foto (Pendeteksian Blob lokal).
- Perakitan struktur pengiriman objek biner ganda (\`FormData\`).
- Sapu bersih *stale cache* tabel menggunakan fungsi \`invalidateQueries\`.

## Verification
- [x] Lolos *Type Checker*.
- [x] Blokir Frontend berfungsi normal (Gagal jika ukuran file melebihi limit 10MB).
- [x] UX Animasi membesar *(Scale)* pada *Hover Container Image*.

## Rules Compliance
- [x] Rule 7.4: Validasi ekstensi dan batas limit dilaksanakan dari sisi antarmuka dan memunculkan Notifikasi *Pop-Up Inline* tanpa merusak aliran *State*.
- [x] Rule 7.2: Memanggil \`api.post\` dan mematuhi skema \`multipart\`.
...
Kita memerlukan gerbang pendaftaran baru. Kita akan mengunggah gambar menggunakan trik *Virtual Blob Memory* dan mengirimkannya melalui *FormData Payload*.

## Task

Kerjakan Issue #033 (Frontend — Employee Create Form).
Ikuti instruksi PERSIS seperti yang tertulis di file:
`docs/issues/033-frontend-employee-create-form.md`

### Urutan Kerja:
1. Jalankan `gh issue create` dan checkout branch git.
2. Buat direktori sub-rute `frontend/src/app/(dashboard)/kepegawaian/create` dan letakkan file `page.tsx` di dalamnya.
3. Salin kode desain Premium Form *(Split Layout)*. Pastikan menambahkan ikon `ChevronDown` di import `lucide-react` bagian atas.
4. Lakukan pengecekan *CSS Grid* dan *Group-Hover* Tailwind untuk memastikan tidak ada kesalahan eja.

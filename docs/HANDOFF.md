...
- [x] Issue #436: Secure Private Storage for Vehicle Documents. PR #436 merged ke `main` (commit `aa18730`). Mengamankan file dokumen kendaraan sensitif (BPKB/STNK) dengan menyimpannya di folder private `private/bmn-documents/`, memblokir akses langsung di Nginx, dan membatasi akses inline preview/view ke pengguna terautentikasi lewat endpoint `/api/bmn/assets/{id}/photo/{type}/view` dengan pengecekan hak akses `bmn.view`. Validasi linter, tsc, dan build pass.
- [x] Issue #434: Descriptive Audit Logging for Observability. PR #435 merged ke `main` (commit `8c0d29c`). Menambahkan pemetaan aksi otomatis `determineAction` pada `AuditLogMiddleware.php` untuk melacak aktivitas sensitif (seperti soft deletes, bulk actions BMN, perubahan wewenang hak akses, login sukses/gagal) dan menyimpannya sebagai properti `_action` di dalam JSON payload. Validasi PHP linter, lint, tsc, dan build pass.
...
**UPDATE SESI CODEX (2026-05-27 - Issue #376 MERGED: Kertas Kerja Analisis Nilai Taksiran BMN):**
...
- **Status**: MERGED (PR #377 -> `878a311`) ke `main`. Remote branch deleted. Belum deploy ke SSH.
- **GitHub**:
  - Issue: #376 `feat(bmn): add asset worksheet form for auction candidates`
  - PR: #377 `feat(bmn): add asset worksheet form (#376)` merged
  - Branch: `issue/376-asset-worksheet-form` deleted on remote
- **Changes sejauh ini**:
...
- **Known issue terakhir dari user**:
  - Tidak ada blocker yang diketahui setelah merge; belum deploy ke production.
- **Files changed / added**:

...
- [ ] Issue #402: Generate BA Pemakaian BMN per pegawai di `/bmn/reports`. Branch `issue/402-ba-pemakaian-bmn` WIP siap PR/testing. Backend tambah tabel `bmn_usage_agreements` + API `GET/POST /api/bmn/usage-agreements` dan snapshot pihak pertama/pihak kedua/aset. Frontend tambah panel BA Pemakaian dengan pilih pegawai, auto-load aset, preview A4 pakai `/header-terbaru.png`, cetak, simpan riwayat, dan daftar history pegawai. Portal pegawai belum dibuat, tapi data history sudah siap difilter `employee_id`. Validasi clean: PHP syntax, route:list, eslint 0 warning, tsc, build 59/59. Belum deploy production.
